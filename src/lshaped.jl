...
function main_problem_lshaped(prices_expected, θ_init, F::Int)
    ...
    @constraint(m, [t in 2:F], b1[t] == b1[t-1] - η1[t] + ξ1[t])
    @constraint(m, b1[1] == 0)
...
function solve_subproblem(b1, η1, ξ1, prices_scenarios)

    m = Model(Gurobi.Optimizer)
    set_silent(m)
    @variable(m, 0 <= b2[1:24-F] <= 800)
    ...
    @constraint(m, b2[1] == b1_link - η2[1] + ξ2[1])
    @constraint(m, [t in 2:(24-F)], b2[t] == b2[t-1] - η2[t] + ξ2[t])
    @constraint(m, c_link, b1_link ==  b1[F])
...
function lshaped(prices_expected, prices_scenarios, prices_prob, θ_init)
    ...
    for k in 1:MAXIMUM_ITERATIONS
        ...
        sub_ret = [
            solve_subproblem(b1_k, eta1_k, xi1_k, prices_scenarios[l]) for l in eachindex(prices_scenarios)
        ]
    ...
    return (lb = LBs, ub = UBs, b1 = value.(b1), η1 = value.(η1), ξ1 = value.(ξ1), b2 = [sub_ret[i].b2 for i in eachindex(prices_scenarios)], η2 = [sub_ret[i].η2 for i in eachindex(prices_prob)], ξ2 = [sub_ret[i].ξ2 for i in eachindex(prices_prob)])
end
...
function multicut_lshaped(prices_expected, prices_scenarios, prices_prob, θ_init)
    ...
    return (lb = LBs, ub = UBs, b1 = value.(b1), η1 = value.(η1), ξ1 = value.(ξ1), b2 = [sub_ret[i].b2 for i in eachindex(prices_scenarios)], η2 = [sub_ret[i].η2 for i in eachindex(prices_scenarios)], ξ2 = [sub_ret[i].ξ2 for i in eachindex(prices_scenarios)])
